...
static EventLoop *CheckLoopNotNull(EventLoop *loop) {
    if (loop == nullptr) {
        LOG_FATAL("%s:%s:%d TcpConncection Loop is null!\n", __FILE__,
...
void TcpConnection::sendInLoop(const void *data, size_t len) {
    ...
    // 之前调用过该connection的shutdonw，不能再进行发送了
    ...
    // 当前这一次write没有把数据全部发送出去，剩余的数据需要保存到缓冲区当中，然后给channel注册epollout事件，
    // poller发现Tcp的发送缓冲区有空间，会通知相应的sock-channel，调用writeCallback_回调方法，也就是调用
    // TcpConnection::handleWrite方法，把发送缓冲区中的数据全部发送完成
